...
describe('Model selection timeout', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    conversationBackend.clear();
    conversationModels.clear();
    ...
    coreMocks.resolveBackendModelId.mockReset();
    coreMocks.resolveBackendModelId.mockImplementation((backend: string, model: string) => {
      if (backend === 'claude' && (model === 'sonnet' || model === 'haiku')) return model;
      return undefined;
    });
  });
  ...
  it('auto-selects the first model after timeout when no history exists', async () => {
    coreMocks.evaluateConversationRunRequest
      .mockReturnValueOnce({ kind: 'ready', conversationId: 'conv-timeout', backend: 'claude' })
      .mockReturnValue({ kind: 'ready', conversationId: 'conv-timeout', backend: 'claude' });
    ...
    // First run: blocked on model selection, card sent
    const result = await runFeishuConversation({
    ...
    // Should have auto-selected 'sonnet' (first model) and resumed
    expect(conversationModels.get('conv-timeout')).toBe('sonnet');
    expect(coreMocks.runPlatformConversation).toHaveBeenCalledWith(
      expect.objectContaining({ conversationId: 'conv-timeout', prompt: 'ship it' }),
    );
  });

  it('auto-selects the previously used model after timeout when history exists', async () => {
    coreMocks.evaluateConversationRunRequest
      .mockReturnValueOnce({ kind: 'ready', conversationId: 'conv-history', backend: 'claude' })
      .mockReturnValue({ kind: 'ready', conversationId: 'conv-history', backend: 'claude' });

    conversationBackend.set('conv-history', 'claude');
    // Pre-set a history model that is NOT yet normalized (simulates stale state)
    conversationModels.set('conv-history', 'haiku');
    ...
    // First run: blocked because model in state doesn't resolve (simulate by clearing after block)
    // Actually: haiku resolves fine, so we need to simulate a case where model was cleared
    conversationModels.delete('conv-history');
    ...
    // Simulate user had previously selected haiku in a prior session
    scheduleModelSelectionTimeout({
      conversationId: 'conv-history',
      transport,
      defaultCwd: process.cwd(),
    ...
    expect(conversationModels.get('conv-history')).toBe('haiku');
    expect(coreMocks.runPlatformConversation).toHaveBeenCalledWith(
      expect.objectContaining({ conversationId: 'conv-history' }),
    );
  });

  it('respects a custom timeout duration', async () => {
    coreMocks.evaluateConversationRunRequest
      .mockReturnValueOnce({ kind: 'ready', conversationId: 'conv-custom', backend: 'claude' })
      .mockReturnValue({ kind: 'ready', conversationId: 'conv-custom', backend: 'claude' });

    conversationBackend.set('conv-custom', 'claude');
    ...
    // Not yet triggered at 4.9s
    await vi.advanceTimersByTimeAsync(4_900);
    expect(coreMocks.runPlatformConversation).not.toHaveBeenCalled();

    // Triggered at 5s
    await vi.advanceTimersByTimeAsync(200);
    expect(coreMocks.runPlatformConversation).toHaveBeenCalled();
  });

  it('cancels the timeout when the user selects a model before it fires', async () => {
    coreMocks.evaluateConversationRunRequest
      .mockReturnValueOnce({ kind: 'ready', conversationId: 'conv-cancel', backend: 'claude' })
      .mockReturnValue({ kind: 'ready', conversationId: 'conv-cancel', backend: 'claude' });

    conversationBackend.set('conv-cancel', 'claude');
    ...
    // User selects model before timeout
    conversationModels.set('conv-cancel', 'sonnet');
    cancel();
